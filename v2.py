...
with st.sidebar:
    ...
    if not df_st.empty:
        ...
        s_id = int(s_data['id'])
        s_books = json.loads(s_data['books']) if s_data['books'] and str(s_data['books']) != 'nan' else []
...
with tab1:
    if st.session_state.edit_id:
        st.warning(f"⚠️ 수정 모드 활성화 중 (ID: {st.session_state.edit_id})")
        if st.button("수정 취소"): 
    ...
    # 폼 외부에서 행 추가/삭제 제어 (Form 안에는 버튼이 submit만 가능하므로)
...
with tab2:
    ...
    if not df_ana.empty:
        ...
        fig = px.line(plot_df, x='x', y='hw_result_rate', markers=True, title=f"{sel_name} 학습 성취도")
...
with tab3:
    st.subheader("📚 교재 관리")
    st.write("현재 등록된 교재:", ", ".join(s_books) if s_books else "없음")
    
    col_add1, col_add2 = st.columns([3, 1])
    nb = col_add1.text_input("추가할 새 교재 이름")
    if col_add2.button("교재 추가") and nb:
    ...
    st.divider()
    if s_books:
        del_book = st.selectbox("삭제할 교재 선택", s_books)
        if st.button("선택 교재 삭제"):
            s_books.remove(del_book)
            df_st.loc[df_st['id'] == s_id, 'books'] = json.dumps(s_books, ensure_ascii=False)
            save_data(df_st, "students")
            st.rerun()
...
with tab4:
    ...
    for _, row in df_log.iterrows():
        with st.expander(f"📌 {int(row['session_num'])}회차 | {row['date']} | 이행률 {row['hw_result_rate']}%"):
            st.write(f"**진도:** {row['progress']}")
            st.write(f"**숙제:** {row['next_hw']}")
            st.info(f"**피드백:** {row['feedback']}")
            
            c_e, c_d = st.columns(2)
            if c_e.button("📝 수정하기", key=f"edit_{row['id']}"):
                st.session_state.edit_id = row['id']
                st.session_state.edit_date = row['date']
                st.session_state.edit_session_num = int(row['session_num'])
                st.session_state.edit_feedback = row['feedback']
                st.rerun()
            if c_d.button("🗑️ 삭제하기", key=f"del_{row['id']}"):
                df_temp = load_data("sessions")
                df_temp = df_temp[df_temp['id'] != row['id']]
                save_data(df_temp, "sessions")
                st.rerun()        s_books = json.loads(s_data['books']) if s_data['books'] else []
