...
def load_data(worksheet_name):
    # ttl="1s"를 주어 1초 동안은 캐시된 데이터를 사용하여 로딩 속도를 개선합니다.
    try:
...
def save_data(df, worksheet_name):
    try:
        ...
        st.cache_data.clear() # 데이터 저장 후 캐시를 비워 최신 상태 유지
    except Exception as e:
        st.error(f"저장 실패: {e}")

# --- [2. 세션 상태 초기화] ---
for key, default in [('p_rows', 1), ('h_rows', 1), ('check_rows', 1), ('edit_id', None)]:
...
with st.sidebar:
    ...
    if not df_st.empty:
        ...
        # 교재 데이터 파싱 안전장치
        raw_books = s_data['books']
        try:
            s_books = json.loads(raw_books) if (pd.notna(raw_books) and raw_books != "") else []
        except:
            s_books = []
...
with tab1:
    ...
    with st.form("lesson_form"):
        ...
        d_val = datetime.strptime(st.session_state.get('edit_date', datetime.now().strftime("%Y-%m-%d")), "%Y-%m-%d") if st.session_state.edit_id else datetime.now()
        n_val = st.session_state.get('edit_session_num', (all_sessions['session_num'].max() + 1 if not all_sessions.empty else 1))
