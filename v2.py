...
with st.sidebar:
    ...
    if not df_st.empty:
        ...
        # 교재 데이터 파싱 (오류 발생 지점 수정 완료)
        raw_books = s_data['books']
        if pd.isna(raw_books) or raw_books == "":
            s_books = []
        else:
            try:
                s_books = json.loads(raw_books)
            except:
                s_books = []
...
with tab1:
    ...
    if c_p2.button("➖ 진도 칸 제거") and st.session_state.p_rows > 1: st.session_state.p_rows -= 1; st.rerun()
    if c_h1.button("➕ 숙제 칸 추가"): st.session_state.h_rows += 1; st.rerun()
    if c_h2.button("➖ 숙제 칸 제거") and st.session_state.h_rows > 1: st.session_state.h_rows -= 1; st.rerun()
