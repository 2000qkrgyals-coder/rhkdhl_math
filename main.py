...
with st.sidebar:
    ...
    if not s_list:
        st.warning("등록된 학생이 없습니다. 사이드바에서 학생을 먼저 등록하세요.")
        st.stop()
...
with tab_input:
    if st.session_state.edit_mode and st.session_state.edit_data is not None:
        ...
        st.warning(f"📍 {pd.to_datetime(ed['date']).strftime('%Y-%m-%d')} ({ed['session']}회차) 수정 모드")
    ...
    if not st.session_state.edit_mode:
        auto_sess = get_next_session(sel_name, sel_date)
    else:
        auto_sess = i_sess
    in_sess = c2.number_input("수업 회차", min_value=1, value=auto_sess, key=f"sess_{u_key}")
    ...
    if st.button("💾 최종 저장 및 노션 공유", type="primary", use_container_width=True):
        ...
        with st.spinner("노션에 공유 중..."):
            notion_data = {"name": sel_name, "date": sel_date.strftime("%Y-%m-%d"), "session": int(in_sess), "hw_df": ed_hw, "pr_df": ed_pr, "memo": in_memo, "nhw_df": ed_nhw, "feedback": in_feed}
            save_to_notion(notion_data)
        
        st.success("✅ 저장 완료!"); trigger_reset(); st.rerun()
