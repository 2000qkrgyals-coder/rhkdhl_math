...
def save_to_notion(data):
    ...
    nhw_summary = "".join([f"• {r.get('분류','')}: {r.get('숙제 범위','')} ({r.get('세부지시','')})\n" for _, r in data['nhw_df'].iterrows()])
    ...
    try: requests.post("https://api.notion.com/v1/pages", headers=headers, json=payload)
    except: pass
...
def get_next_session(name, date_obj):
    ...
    return 1 if res[0] is None else int(res[0]) + 1
...
tab_input, tab_search, tab_analysis = st.tabs(["📝 수업 기록", "🔍 기록 조회", "📊 성취도 분석"])
...
with tab_input:
    ...
    if st.session_state.edit_mode:
        ...
        i_hw, i_pr = pd.read_json(io.StringIO(ed['homeworks'])), pd.read_json(io.StringIO(ed['progress_list']))
        i_memo = json.loads(ed['solved_problems'])[0]['요약']
        i_feed, i_nhw = ed['feedback'], pd.read_json(io.StringIO(ed['next_hw_list']))
        i_st, i_et = datetime.strptime(ed['start_time'], "%H:%M").time(), datetime.strptime(ed['end_time'], "%H:%M").time()
    else:
        i_date = datetime.now()
        # --- 자동 회차 계산 적용 ---
        ...
        i_memo, i_feed, i_nhw = "", "", pd.DataFrame(columns=["분류", "숙제 범위", "세부지시"])
    ...
    # 날짜가 바뀌면 회차를 다시 계산하여 반영
    calc_sess = i_sess if st.session_state.edit_mode else get_next_session(sel_name, sel_date)
    in_sess = c2.number_input("수업 회차", min_value=1, value=calc_sess, key=f"s_{u_key}")
    
    in_st = c3.time_input("시작", i_st, key=f"st_{u_key}")
    in_et = c4.time_input("종료", i_et, key=f"et_{u_key}")

    st.divider()
    st.markdown("#### 2️⃣ 오늘 숙제 달성도")
    if not st.session_state.edit_mode and not all_recs.empty and st.button("💡 직전 숙제 양식 가져오기"):
        last_nhw = pd.read_json(io.StringIO(all_recs.sort_values(['date', 'session']).iloc[-1]['next_hw_list']))
        st.session_state.temp_hw_data = pd.DataFrame({"분류": last_nhw["분류"], "범위": last_nhw["숙제 범위"], "총 문항":0, "푼 문항":0, "모름":0, "안함":0})
        st.rerun()
    ...
    in_memo = st.text_area("수업 메모", value=i_memo, key=f"m_{u_key}")

    st.divider()
    st.markdown("#### 4️⃣ 다음 숙제 및 피드백")
    ed_nhw = st.data_editor(i_nhw, num_rows="dynamic", use_container_width=True, key=f"nh_{u_key}", column_config={"분류": st.column_config.SelectboxColumn("교재", options=curr_books)})
    ...
    if st.button("💾 최종 저장 및 노션 공유", type="primary", use_container_width=True):
        ...
        memo_j, dur = json.dumps([{"요약": in_memo}]), (datetime.combine(sel_date, in_et) - datetime.combine(sel_date, in_st)).seconds / 3600
        ...
        st.success("✅ 저장 및 공유 완료!"); trigger_reset(); st.rerun()
...
with tab_search:
    if not all_recs.empty:
        ...
        sel_v = st.selectbox("조회 기록 선택", v_list)
...
with tab_analysis:
    if not all_recs.empty:
        ...
        for _, r in all_recs.iterrows():
            ...
            analysis_list.append({"날짜": r['date'], "회차": f"{r['session']}회", "성취도": round(sol/tot*100, 1) if tot > 0 else 0})
